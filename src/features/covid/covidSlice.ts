...
const apiiUrl = 'https://covid19.mathdro.id/api'
...
const initialState: covidState = {
  data: {
    ...
    lastUpdate: '2020-09-02T14:28:26.000Z',
  },
  country: '',
  dailyData: [
    {
...
export const fetchAsyncGet = createAsyncThunk('covid/get', async () => {
  const { data } = await axios.get<APIDATA>(apiiUrl)
  return data
})
...
export const fetchAsyncGetDaily = createAsyncThunk(
  'covid/getDaily',
  async () => {
    const { data } = await axios.get<APIDATADAILY>(`${apiiUrl}/daily`)
    return data
  }
)
...
export const fetchAsyncGetCountry = createAsyncThunk(
  ...
  async (country: string) => {
    let dynamicUrl = apiiUrl
    if (country) {
      dynamicUrl = `${apiiUrl}/countries/${country}`
    }
...
const covidSlice = createSlice({
  ...
  extraReducers: (builder) => {
    // fulfilled 処理が正常処理されて時に帰ってくるstate
